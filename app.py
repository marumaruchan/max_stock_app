...
"インジケーター（複数選択可）",
["SMA 20", "EMA 50", "Bollinger Bands", "RSI 14", "VWAP Band 20", "VWAP Band 40"],
default=[]
)
...
def add_indicators(df: pd.DataFrame, inds: list) -> pd.DataFrame:
    ...
    if "VWAP Band 20" in inds:
        ...
        vwap_20 = volume_price.rolling(20).sum() / df["Volume"].rolling(20).sum()

        # VWAPからの価格偏差を計算
        price_deviation = typical_price - vwap_20
        price_variance = (price_deviation ** 2 * df["Volume"]).rolling(20).sum() / df["Volume"].rolling(20).sum()
        vwap_std_20 = np.sqrt(price_variance)

        df["VWAP20"] = vwap_20
        df["VWAP20_upper_1s"] = vwap_20 + vwap_std_20
        df["VWAP20_lower_1s"] = vwap_20 - vwap_std_20
        df["VWAP20_upper_2s"] = vwap_20 + 2 * vwap_std_20
        df["VWAP20_lower_2s"] = vwap_20 - 2 * vwap_std_20
    ...
    if "VWAP Band 40" in inds:
        ...
        typical_price = (df["High"] + df["Low"] + df["Close"]) / 3
        volume_price = typical_price * df["Volume"]
        vwap_40 = volume_price.rolling(40).sum() / df["Volume"].rolling(40).sum()

        # VWAPからの価格偏差を計算
        price_deviation = typical_price - vwap_40
        price_variance = (price_deviation ** 2 * df["Volume"]).rolling(40).sum() / df["Volume"].rolling(40).sum()
        vwap_std_40 = np.sqrt(price_variance)

        df["VWAP40"] = vwap_40
        df["VWAP40_upper_1s"] = vwap_40 + vwap_std_40
        df["VWAP40_lower_1s"] = vwap_40 - vwap_std_40
        df["VWAP40_upper_2s"] = vwap_40 + 2 * vwap_std_40
        df["VWAP40_lower_2s"] = vwap_40 - 2 * vwap_std_40
